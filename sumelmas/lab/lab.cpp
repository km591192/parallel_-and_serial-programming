...
//паралельное
int sum_arrparal(int *a, const int n, int cproc)
{
	int sum = 0;

	//omp_set_dynamic(0);      // Не менять число потоков во время исполнения
	omp_set_num_threads(cproc); // число потоков
...
int main(int argc, char **argv)
{
	...
	for (n = 1; n <= 64; n *= 2)
	{
		...
		cout << "------------------------\n";
		*/
		sum_arrparal(a, 10000,n);
		cout << "<<<<<<<<<<<<<<<<<<<<<<" << endl << endl;
		sum_arrparal(a, 100000, n);
		cout << "<<<<<<<<<<<<<<<<<<<<<<<" << endl << endl;
		sum_arrparal(a, 1000000, n);
		cout << "<<<<<<<<<<<<<<<<<<<<<<<" << endl << endl;
		sum_arrparal(a, 10000000, n);
		cout << "<<<<<<<<<<<<<<<<<<<<<<<" << endl << endl;
		sum_arrparal(a, 100000000, n);
		cout << "---------------------------------------------" << endl << endl;
